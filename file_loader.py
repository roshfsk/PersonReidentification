                    ...
                    if findex >= 0:                        
                        ...
                        if intemplatefileindex == len(files)-1:
                            if len(files) < len(pepsList):
                                ...
                                newsavepath = r'' + roipath 
                                if not os.path.exists(newsavepath):
                                    os.makedirs(newsavepath)
                                cv.imwrite(newsavepath + '1_' + str(pid) + ".png", roi)
                                break
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            if max_val2 >= threshold2 : 
                ...
                pidfile = splittedpath[len(splittedpath)-1]
                splittedid = pidfile.split('_')
                person_id_png = splittedid[len(splittedid)-1]
                newsavepath = r'' + roipath
                if not os.path.exists(newsavepath):
                    os.makedirs(newsavepath)
                cv.imwrite(newsavepath + str(fileIndex) + '_' + person_id_png, matcherroi)
...
#---------------------
        ...
        if not os.path.exists(newsavepath):
            os.makedirs(newsavepath)
        cv.imwrite(newsavepath + str(fileIndex) + '_' + str(pid) + ".png", roi)


# if fileIndex > 1:
#     templateloadpath = r'' + roipath + str(fileIndex-1) +'_'
# else:
#     templateloadpath = r'' + roipath + str(fileIndex) +'_'
templateloadpath = r'' + roipath
files=[]
if fileIndex > 1:
    files = glob.glob( os.path.join(templateloadpath, str(fileIndex-1) + '_*.*'))
else:
    files = glob.glob( os.path.join(templateloadpath, str(fileIndex) + '_*.*'))
#files.sort()
for intemplatefile in files:
    ...
    pidfile = splittedpath[len(splittedpath)-1]
    splittedid = pidfile.split('_')
    person_id = splittedid[len(splittedid)-1]
